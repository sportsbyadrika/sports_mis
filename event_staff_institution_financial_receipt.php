    ...
    <div class="receipt-wrapper">
        ...
        <section class="meta">
            <div>
                ...
                <div><?php echo receipt_sanitize($institution['name']); ?></div>
            </div>
            <div>
                <strong>Institution ID</strong>
                <div>#<?php echo receipt_sanitize((string) $institution['id']); ?></div>
            </div>
            <div>
                <strong>Total Fee Due</strong>
                <div>₹<?php echo number_format($snapshot['total_fee_due'], 2); ?></div>
            </div>
            <div>
                <strong>Total Received</strong>
                <div>₹<?php echo number_format($snapshot['fund_total'], 2); ?></div>
            </div>
        </section>
        ...
        <section class="meta" style="margin-top: 32px;">
            <?php if (!empty($event['bank_name']) || !empty($event['bank_account_number'])): ?>
                <div>
                    <strong>Bank Details</strong>
                    <?php if (!empty($event['bank_name'])): ?>
                        <div><?php echo receipt_sanitize($event['bank_name']); ?></div>
                    <?php endif; ?>
                    <?php if (!empty($event['bank_account_number'])): ?>
                        <div>Account No: <?php echo receipt_sanitize($event['bank_account_number']); ?></div>
                    <?php endif; ?>
                    <?php if (!empty($event['bank_ifsc'])): ?>
                        <div>IFSC: <?php echo receipt_sanitize($event['bank_ifsc']); ?></div>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
            <div>
                <strong>Generated By</strong>
                <div><?php echo receipt_sanitize($user['name'] ?? 'Event Staff'); ?></div>
                <div class="small-note">Receipt generated via Sports MIS</div>
            </div>
        </section>
