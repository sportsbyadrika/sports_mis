...
if ($selected_age_category_id !== null) {
    $sql = "SELECT em.id, em.name, em.label, em.gender, ac.name AS age_category,
                   COUNT(DISTINCT CASE WHEN ier.status = 'approved' THEN ier.institution_id END) AS approved_institutions,
                   COUNT(DISTINCT CASE WHEN ier.status = 'approved' AND res.result IS NOT NULL AND res.result <> '' THEN ier.institution_id END) AS results_updated,
                   COALESCE(st.status, 'pending') AS result_status
            FROM event_master em
            INNER JOIN age_categories ac ON ac.id = em.age_category_id
            LEFT JOIN institution_event_registrations ier ON ier.event_master_id = em.id
            LEFT JOIN institution_event_results res ON res.event_master_id = em.id AND res.institution_id = ier.institution_id
            LEFT JOIN institution_event_result_statuses st ON st.event_master_id = em.id
            WHERE em.event_id = ? AND em.event_type = 'Institution'
                AND em.age_category_id = ?
                AND COALESCE(st.status, 'pending') = ?
            GROUP BY em.id, em.name, em.label, em.gender, ac.name, st.status
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="h4 mb-0">Institution Result Entry</h1>
        <p class="text-muted mb-0">Manage institution level results and publication statuses.</p>
    </div>
</div>
<div class="card shadow-sm">
    <div class="card-body">
        <?php if (!$age_categories): ?>
            <p class="mb-0 text-muted">No institution events are configured for result entry.</p>
        <?php else: ?>
            ...
            <?php if (count($events) === 0): ?>
                <p class="mb-0 text-muted">No institution events match the selected filters.</p>
            <?php else: ?>
                ...
                <table class="table table-striped align-middle">
                    <thead>
                        <tr>
                            ...
                            <th>Event Label</th>
                            <th>Gender</th>
                            <th class="text-center">Approved Institutions</th>
                    ...
                    <tbody>
                        <?php foreach ($events as $index => $event): ?>
                            ...
                            <tr>
                                ...
                                <td class="text-center"><?php echo (int) $event['approved_institutions']; ?></td>
                                <td class="text-center">
                                    <?php
                                    $results_updated = (int) $event['results_updated'];
                                    $results_badge_class = $results_updated === (int) $event['approved_institutions']
                                        ? 'bg-success'
                                ...
                                <td class="text-end">
                                    <a class="btn btn-sm btn-outline-primary" href="result_institution_entry.php?event_master_id=<?php echo (int) $event['id']; ?>" title="Update Results">
